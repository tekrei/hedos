...
public class LinKernighanOptimization extends LocalSearchOptimizer {
    ...
    private boolean checkLKImprovement(int[] genes, int t1Idx, MemorySegment dist, int[][] neighborLists, int n) {
        int t1 = genes[t1Idx];
        int t2Idx = (t1Idx + 1) % n;
        int t2 = genes[t2Idx];
        float initialGain = dist.getAtIndex(ValueLayout.JAVA_FLOAT, (long) t1 * n + t2);

        for (int t3 : neighborLists[t2]) {
            float gain = initialGain - dist.getAtIndex(ValueLayout.JAVA_FLOAT, (long) t2 * n + t3);
            if (gain <= 0) continue;
            ...
            if (gain + dist.getAtIndex(ValueLayout.JAVA_FLOAT, (long) t3 * n + t4) - 
                       dist.getAtIndex(ValueLayout.JAVA_FLOAT, (long) t4 * n + t1) > 0.001f) return true;
        }
        return false;
    }

    private void applyLKMove(int[] genes, int t1Idx, MemorySegment dist, int[][] neighborLists, int n, int[] posMap) {
        int t1 = genes[t1Idx];
        int t2Idx = (t1Idx + 1) % n;
        int t2 = genes[t2Idx];
        ...
        for (int t3 : neighborLists[t2]) {
            ...
            if (gain + dist.getAtIndex(ValueLayout.JAVA_FLOAT, (long) t3 * n + t4) - 
                       dist.getAtIndex(ValueLayout.JAVA_FLOAT, (long) t4 * n + t1) > 0.001f) {
